...
class GarminClient(object):
    ...
    @require_session
    def get_daily_summary(self, displayname, date):
        response = self.session.get("https://connect.garmin.com/modern/proxy/usersummary-service/usersummary/daily/{}?calendarDate={}".format(displayname, date))
        if response.status_code != 200:
            raise Exception(u"failed to fetch daily summary: {}\n{}".format(response.status_code, response.text))
        return response.text

    @require_session
    def get_daily_summaries(self, displayname, start=1, limit=100):
        # 'start' indicates how many days before maxDate we want to start. 1 = today, 2 is yesterday etc.
        # 'limit indicates how many items should be retrieved.
        # 'maxDate' is the last date for which you want activites.
        entries = []
        # end_date = yesterday, as we don't want to get activities for a day still in progress.
        end_date = datetime.today() - timedelta(days=1)

        while True:
            response = self.session.get("https://connect.garmin.com/modern/proxy/usersummary-service/usersummary/list/{}?start={}&maxDate={}&limit={}".format(displayname, start, end_date, limit))

            json_response = json.loads(response.text)
            if len(json_response) > 1:
                dates = []
                for item in json_response:
                    day = datetime.strptime(item['calendarDate'], '%Y-%m-%d')
                    dates.append(day)
                end_date = min(dates).strftime("%Y-%m-%d")

                if response.status_code != 200:
                    raise Exception(u"failed to fetch daily info: {}\n{}".format(response.status_code, response.text))
                else:
                    entries.append(json_response)
            # Once we get 1 response, we've hit the last activity.
            else:
                break
        return entries
    ...
    def get_displayname(self):
        ...
        match = re.search(r"displayName\\\":\\\"([^\\]*)\\\"", response.text)
        if not match:
